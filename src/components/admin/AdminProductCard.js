    ...
    <Card sx={{ marginLeft:1, maxWidth: 280 }}>
      ...
      <CardContent>
        ...
        <Typography variant="body1" color="text.primary">
         sizes: {sizes.map(s => <span>{s.data.size + " "}</span> )}
        </Typography>
      </CardContent>
      <CardActions disableSpacing={true}>
        <Link  style={{textDecoration:"none"}} to={"/admin/product/"+product.slug}>
            <Button color="success" size="large">Update</Button>
        </Link>
        <Button color="success" size="large" onClick={() => handleRemove(item.id)}>Delete</Button>
      </CardActions>
    </Card>
  );
}